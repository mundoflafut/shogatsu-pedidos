# v30 — Correções do relatório de auditoria (2 críticos, 3 altos, 3 médios, 1 baixo)

Esta versão resolve os itens levantados no relatório de auditoria, por ordem de criticidade.

**🔴 C1 — Sessões de login não sumem mais a cada restart.** Antes ficavam só na memória do
processo Node; qualquer deploy, ou o site "acordando" no plano Free do Render, derrubava todo
mundo sem aviso. Agora as sessões são salvas em `data/sessions.json` (e sincronizadas no Supabase,
reaproveitando o mecanismo que já existia pra pedidos/cardápio/clientes) toda vez que mudam, e
recarregadas quando o servidor liga de novo. Testado de ponta a ponta: login → simulação de
restart do processo → mesmo token continua válido, sem precisar logar de novo. Também adicionei
`POST /api/logout` (encerra a sessão no servidor, não só localStorage) e `GET /api/whoami`
(checagem leve de token, usada no item C2 abaixo).

**🔴 C2 — Indicador visual de conexão em tempo real no painel.** Antes o `EventSource` do painel
não tinha handler de `onopen`/`onerror` — se a conexão caísse, a tela continuava parecendo normal
e pedidos novos podiam parar de aparecer sem ninguém perceber. Agora tem uma bolinha "● Ao vivo /
● Reconectando..." no topo do painel, ligada aos eventos reais da conexão; depois de algumas
tentativas seguidas falhando, o painel confirma se o token ainda é válido (via `/api/whoami`) e,
se não for, faz logout automático em vez de ficar tentando reconectar pra sempre.

**🟠 A1 — Proteção contra força bruta no login.** `/api/login` agora bloqueia por IP depois de 5
tentativas erradas em 10 minutos, com atraso crescente (30s → 1min → 2min... até um teto de
15min), resetando ao acertar a senha. Testado: 5 tentativas erradas seguidas → a 6ª já vem
bloqueada com HTTP 429 e `Retry-After`.

**🟠 A2 — Painel responsivo (tablet/celular).** O arquivo não tinha nenhuma `@media query`. Agora,
abaixo de 1024px a barra lateral vira uma gaveta (abre com o botão ☰, fecha sozinha ao trocar de
página ou tocar fora); abaixo de 768px os painéis/grades reorganizam em menos colunas (dashboard,
kanban, relatórios, configurações, itens do cardápio); abaixo de 480px viram coluna única.

**🟠 A3 — Requisição grande não trava mais a conexão sem resposta.** Quando o corpo passava de
8MB, o servidor destruía a conexão mas a Promise de `readBody()` nunca era resolvida nem
rejeitada — o cliente ficava pendurado até dar timeout. Agora rejeita explicitamente assim que
detecta o excesso de tamanho.

**🟡 M3 — Número do pedido (senha) não depende mais de regravar o cardápio inteiro.** Isolado num
arquivo próprio (`data/ticket-counter.json`), com migração automática do valor que já estava
salvo dentro do `config.json` em instalações antigas — ninguém perde o número em que o ciclo
estava. Antes, atribuir um número relia e regravava o `config.json` completo (cardápio incluso),
o que era frágil por natureza mesmo com janela de risco pequena.

**🟡 M4 — Zoom liberado no site do cliente.** Removido `maximum-scale=1.0` do viewport, que
impedia dar zoom com dois dedos — problema de acessibilidade pra quem tem dificuldade visual.

**🟡 B4 — Avaliações públicas agora paginadas de verdade.** `GET /api/reviews` aceita
`?offset=&limit=` (limite máx. 20 por página) em vez de sempre calcular e mandar até 50 de uma
vez; o site do cliente carrega as primeiras 20 e mostra um botão "Carregar mais avaliações" pra
buscar o resto sob demanda. A nota média e o total continuam batendo com TODAS as avaliações
visíveis, não só a página atual.

**⚪ B1 — Log centralizado (parcial).** Criei uma função `log(nível, contexto, mensagem)` única e
troquei por ela os pontos de Supabase, geocodificação (BrasilAPI/Nominatim), Twilio (WhatsApp
automático) e o webhook do Mercado Pago — inclusive um caso onde o comentário dizia "erro fica no
log" mas nada era de fato registrado.

**Itens do relatório sem alteração de código nesta versão:** M1 (confirmar se
`SUPABASE_URL`/`SUPABASE_SERVICE_KEY` estão configuradas no Render — isso é uma checagem sua, o
mecanismo já existe) · M2 (já mitigado com Photon/BrasilAPI como já estava) · M5 (pede validação
manual num celular real, não é algo que eu resolva por código) · B2 (pede impressora física de
verdade) · B3 (nenhuma ação urgente recomendada, só ponto de atenção).

...
Usa só duas dependências externas: `pg` (opcional, clientes em Postgres) e
`web-push` (notificações grátis no app). Rode `npm install` antes de
`node server.js` na primeira vez.

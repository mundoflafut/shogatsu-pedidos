...
// AI Assistant (Gemini) — helps the admin with quick tasks: sales summaries,
// finding orders, drafting customer messages, suggesting menu/promo ideas, etc.
...
app.post('/api/admin/ai-assistant', async (req, res) => {
  if (!requireRole(getTokenFromReq(req), 'admin')) return res.status(403).json({ error: 'Seu usuário não tem permissão.' });
  const ai = getGenAI();
  if (!ai) return res.status(400).json({ error: 'GEMINI_API_KEY não configurada no servidor.' });

  const question = String(req.body?.message || '').slice(0, 2000);
  if (!question.trim()) return res.status(400).json({ error: 'Mensagem vazia.' });
  ...
  try {
    ...
    const abertos = orders.filter((o: any) => !['entregue', 'cancelado'].includes(o.status));
    const contextSummary = {
      loja: cfg.name,
      pedidosHoje: todayOrders.length,
      faturamentoHoje: todayOrders.filter((o: any) => o.status !== 'cancelado').reduce((s: number, o: any) => s + (o.total || 0), 0).toFixed(2),
      pedidosEmAndamento: abertos.length,
      ultimosPedidos: orders.slice(0, 8).map((o: any) => ({
        id: o.id, status: o.status, nome: o.name, total: o.total, criadoEm: o.createdAt
      }))
    };

    const systemPrompt = `Você é a assistente de IA do painel administrativo do restaurante "${cfg.name}" (sistema de pedidos online Shogatsu). ` +
      `Ajude o admin com tarefas rápidas: resumir vendas, localizar pedidos, sugerir respostas a clientes, redigir mensagens de campanha/promoção, ` +
      `analisar o desempenho do dia, e tirar dúvidas sobre como usar o painel. Seja direta, objetiva, em português do Brasil. ` +
      `Quando fizer sentido, use os dados de contexto abaixo (não invente números que não estão aqui). ` +
    ...
    const text = response.text || 'Não consegui gerar uma resposta agora.';
    res.json({ ok: true, reply: text });
  } catch (e: any) {
    console.error('AI assistant error', e);
    res.status(500).json({ error: 'Falha ao consultar a IA: ' + (e?.message || 'erro desconhecido') });
  }
});

app.delete('/api/orders/:id', (req, res) => {
  if (!requireRole(getTokenFromReq(req), 'admin')) return res.status(403).json({ error: 'Seu usuário não tem permissão para excluir pedidos.' });
  const id = req.params.id;
  const orders = readJSON(ORDERS_FILE, []);

// ═══════════════════════════════════════════════════════════
// SHOGATSU · Servidor de Pedidos Online — FIXED VERSION
// Fixes aplicados:
// 1. url.parse() → URL API nativa (remove deprecation warning)
// 2. url.URLSearchParams → URLSearchParams nativo
// 3. Meta tag PWA fix no HTML
// ═══════════════════════════════════════════════════════════
...
// ✅ FIX 1: Substituir url.parse() (DEPRECATED) por URL API nativa
...
function sendSMS(toPhone, body, smsCfg) {
  return new Promise((resolve, reject) => {
    if (!smsCfg.accountSid || !smsCfg.authToken || !smsCfg.fromNumber) {
      return reject(new Error('SMS não configurado.'));
    }
    const to = String(toPhone || '').replace(/\D/g, '');
    if (!to) return reject(new Error('Número de telefone inválido.'));
    // ✅ FIX 2: URLSearchParams nativo (não deprecated)
    const params = new URLSearchParams({ To: '+55' + to, From: smsCfg.fromNumber, Body: body }).toString();
...
const server = http.createServer(async (req, res) => {
  // ✅ FIX: Usar parseUrl() em vez de url.parse() (deprecated)
  ...
  if (pathname === '/api/change-password' && req.method === 'POST') {
    ...
    try {
      ...
      if (!next || next.length < 4) return sendJSON(res, 400, { error: 'nova senha muito curta' });
      data.cfg[field] = next;
  ...
  if (pathname === '/api/push/subscribe' && req.method === 'POST') {
    try {
      ...
      if (!phone || !subscription || !subscription.endpoint) return sendJSON(res, 400, { error: 'dados inválidos' });
  ...
  // ── PATCH /api/admin/campaigns/messages/:id ──
  ...
  // ── GET /api/admin/campaigns/schedule ──
  ...
  // ── POST /api/admin/campaigns/schedule ──
  if (pathname === '/api/admin/campaigns/schedule' && req.method === 'POST') {
    if (!requireRole(getToken(req, query), 'admin')) return sendJSON(res, 403, { error: 'sem permissão' });
    try {
      const { horarios } = await readBody(req);
      const validos = (Array.isArray(horarios) ? horarios : []).filter((h) => /^([01]
|2[0-3]):[0-5]
$/.test(h));
  ...
  // ── POST /api/admin/campaigns/send-now ──
  ...
  if (pathname === '/api/admin/users' && req.method === 'GET') {
    if (!requireRole(getToken(req, query), 'master')) return sendJSON(res, 403, { error: 'Só o master pode gerenciar usuários.' });
    const { cfg } = await readConfig();
  ...
  if (pathname === '/api/pix' && req.method === 'POST') {
    try {
      ...
      const { cfg } = await readConfig();
      if (!cfg.pixKey) return sendJSON(res, 400, { error: 'PIX não configurado' });
  ...
  if (pathname.match(/^\/api\/cep\/[^/]+$/) && req.method === 'GET') {
    ...
    } catch (e) { return sendJSON(res, 200, { ok: false, error: 'Não foi possível buscar esse CEP.' }); }
  }
  ...
  if (pathname === '/api/reverse-geocode' && req.method === 'POST') {
    try {
      ...
      if (!addr) return sendJSON(res, 200, { ok: false, error: 'Não conseguimos identificar seu endereço.' });
      return sendJSON(res, 200, { ok: true, ...addr });
    } catch (e) { return sendJSON(res, 200, { ok: false, error: 'Não conseguimos identificar seu endereço.' }); }
  }
  ...
  // ── GET /api/admin/detect-usb-printers ──
  if (pathname === '/api/admin/detect-usb-printers' && req.method === 'GET') {
    ...
    return sendJSON(res, 200, {
      found: candidates,
      note: candidates.length ? null : 'Nenhuma impressora USB encontrada. Na nuvem (Render), isso é esperado.'
    });
  }

  // ── GET /api/admin/detect-network-printers ──
  if (pathname === '/api/admin/detect-network-printers' && req.method === 'GET') {
    ...
    if (!base) {
      return sendJSON(res, 200, { found: [], note: 'Não foi possível identificar uma rede local Wi-Fi — normal na nuvem (Render).' });
    }
  ...
  if (pathname === '/api/admin/geocode-store' && req.method === 'POST') {
    ...
    try {
      ...
      if (!geo) return sendJSON(res, 404, { error: 'Não conseguimos localizar esse endereço.' });
      data.cfg.storeLat = geo.lat;
      data.cfg.storeLng = geo.lng;
      await writeConfig(data);
      return sendJSON(res, 200, { lat: geo.lat, lng: geo.lng, label: geo.label });
    } catch (e) { return sendJSON(res, 500, { error: 'Erro ao localizar endereço.' }); }
  }
  ...
  if (pathname === '/api/admin/restore' && req.method === 'POST') {
    if (!requireRole(getToken(req, query), 'master')) return sendJSON(res, 403, { error: 'Só o master pode restaurar.' });
    try {
      const body = await readBody(req);
      if (!body || body.version !== 1 || !body.cfg || !body.menu) {
        return sendJSON(res, 400, { error: 'Arquivo de backup inválido.' });
      }
      const current = await readConfig();
      await writeConfig({
...
function estimateDeliveryWindow(order, cfg) {
  ...
  // ═══════════════════════════════════════════════════════════
  // ✅ FIX 3: LOGIN DO CLIENTE — com validação e logs de debug
  // ═══════════════════════════════════════════════════════════
  if (pathname === '/api/customer/login' && req.method === 'POST') {
    try {
      ...
      // 🐛 LOG DE DEBUG — ver o que está chegando no body
      console.log('[DEBUG /api/customer/login] body:', JSON.stringify(body));
      console.log('[DEBUG /api/customer/login] phone:', phone, '| pin type:', typeof pin, '| pin:', pin);

      // Validação: verifica se phone e pin foram enviados
      if (!phone) {
        console.log('[DEBUG /api/customer/login] ❌ ERRO: phone não enviado');
        return sendJSON(res, 400, { error: 'Informe o telefone.' });
      }
      if (!pin) {
        console.log('[DEBUG /api/customer/login] ❌ ERRO: pin não enviado');
        return sendJSON(res, 400, { error: 'Informe a senha de 4 dígitos.' });
      }

      const p = normalizePhone(phone);
      if (p.length < 10) {
        console.log('[DEBUG /api/customer/login] ❌ ERRO: telefone inválido:', p);
        return sendJSON(res, 400, { error: 'Telefone inválido.' });
      }

      const customer = await customerDB.findCustomerByPhone(p);
      if (!customer) {
        console.log('[DEBUG /api/customer/login] ❌ ERRO: cliente não encontrado:', p);
        return sendJSON(res, 401, { error: 'Telefone ou senha incorretos.' });
      }

      const hash = hashPin(p, pin);
      if (customer.pinHash !== hash) {
        console.log('[DEBUG /api/customer/login] ❌ ERRO: pinHash não bate');
        console.log('[DEBUG /api/customer/login] esperado:', customer.pinHash);
        console.log('[DEBUG /api/customer/login] recebido:', hash);
        return sendJSON(res, 401, { error: 'Telefone ou senha incorretos.' });
      }

      console.log('[DEBUG /api/customer/login] ✅ SUCESSO:', customer.phone, customer.name);
      const orders = await db.getOrders();
      ...
      console.error('[DEBUG /api/customer/login] 💥 ERRO GERAL:', e.message);
      return sendJSON(res, 400, { error: 'invalid body' }); 
    }
  }

  // ── POST /api/customer/recovery-request ──
  ...
  // ── POST /api/customer/recovery-set-pin ──
  if (pathname === '/api/customer/recovery-set-pin' && req.method === 'POST') {
    try {
      ...
      if (!customer.recovery.approved) {
        return sendJSON(res, 403, { error: 'Ainda aguardando a confirmação da loja pelo WhatsApp.' });
      }
  ...
  // ── POST /api/admin/customers/recovery-approve ──
  if (pathname === '/api/admin/customers/recovery-approve' && req.method === 'POST') {
    ...
    try {
      ...
      const p = normalizePhone(phone);
      const customer = await customerDB.findCustomerByPhone(p);
      if (!customer || !customer.recovery) return sendJSON(res, 404, { error: 'Nenhuma recuperação pendente.' });
    ...
    try {
      ...
      if (ticketNumber !== undefined && ticketNumber !== null && ticketNumber !== '') {
        ...
        if (n >= 1 && n <= 200) {
          ...
          if (conflict) {
            return sendJSON(res, 400, { error: `O número ${n} já está sendo usado pelo pedido em andamento ${conflict.id}.` });
          }
  ...
  // ── PATCH /api/admin/reviews/:orderId ──
...
db.initSchema()
  .then(() => {
    console.log('🗄️  Conectado ao Postgres — cardápio, configurações, pedidos e clientes 100% no banco.');
  })
  .catch(err => {
    console.error('⚠️  Erro ao conectar/preparar o banco de dados:', err.message);
    console.error('   Confira a variável DATABASE_URL.');
    process.exit(1);
  })
  .then(() => {
    server.listen(PORT, async () => {
      ...
      if (vapidOk) console.log('🔔 Web Push configurado.');
      else if (notifications.webpushDisponivel()) console.log('🔕 Web Push instalado mas sem chaves VAPID.');
      else console.log('🔕 Pacote "web-push" não instalado.');
    });

    // Agendador de campanhas automáticas
    ...
    setInterval(async () => {
      ...
      if (horarios.includes(agora)) {
        ...
        } catch (e) { console.error('[campanhas] erro:', e.message); }
      }
    }, 60 * 1000);
  });

...
// Node.js quase puro — http, fs, crypto — + 'pg' pra clientes em Postgres
// (opcional: sem DATABASE_URL configurada, cai automaticamente pro arquivo
// customers.json, sem quebrar nada)
// ═══════════════════════════════════════════════════════════
...
const DEFAULT_CFG = {
  ...
  // ── Web Push (notificações grátis pro app instalado) — gere as chaves uma vez com
  // node -e "console.log(require('web-push').generateVAPIDKeys())" e cole aqui.
...
// Envia um SMS via Twilio (precisa de conta própria configurada em Configurações → SMS).
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/admin/campaigns' && req.method === 'GET') {
    if (!checkAuth(getToken(req, query))) return sendJSON(res, 401, { error: 'unauthorized' });
    return sendJSON(res, 200, notifications.readCampaigns());
  }
...
customerDB.initSchema()
  .then(() => {
    if (customerDB.useDB) console.log('🗄️  Conectado ao Postgres — clientes armazenados no banco (persistem entre reinícios).');
    else console.log('📄 DATABASE_URL não configurada — clientes continuam salvos em data/customers.json.');
  })
  .catch(err => console.error('⚠️  Erro ao conectar/preparar o banco de dados:', err.message))
  .finally(() => {
    server.listen(PORT, () => {

...
function estimateDeliveryWindow(order, cfg) {
  ...
  if (pathname === '/api/print' && req.method === 'POST') {
    ...
    try {
      ...
      if (order.ticketNumber) lines.push(ESC.center + ESC.boldOn + ESC.doubleOn + 'SENHA ' + order.ticketNumber + ESC.doubleOff + ESC.boldOff + ESC.left);
  ...
  if (pathname === '/api/orders' && req.method === 'POST') {
    try {
      ...
      const orders = readJSON(ORDERS_FILE);
      const ticketNumber = Number(cfg.nextTicketNumber) >= 1 && Number(cfg.nextTicketNumber) <= 200 ? Number(cfg.nextTicketNumber) : 1;
      const order = {
      ...
      // Avança o contador da senha pro próximo pedido, voltando pra 1 depois de passar de 200
      const cfgData = readConfig();
      cfgData.cfg.nextTicketNumber = ticketNumber >= 200 ? 1 : ticketNumber + 1;
  ...
  if (pathname.startsWith('/api/orders/') && req.method === 'PATCH') {
    ...
    try {
      ...
      if (ticketNumber !== undefined && ticketNumber !== null && ticketNumber !== '') {
        ...
        if (n >= 1 && n <= 200) order.ticketNumber = n;
      }
      if (status === 'cancelado') {
